...
def start(update: Update, context: CallbackContext):
    ...
    if existing_user:
        ...
        if buttons:
            ...
            # No ticket functionality - just send registration photo
            try:
                with open('img/free_shot.jpg', 'rb') as photo:
                    update.message.reply_photo(
                        photo=photo,
                        caption="Приходи на тусовку и забирай бесплатный шот!"
                    )
...
def handle_contact(update: Update, context: CallbackContext):
    ...
    # Send the specific photo after registration
    try:
        with open('img/free_shot.jpg', 'rb') as photo:
            update.message.reply_photo(
                photo=photo,
        ...
        update.message.reply_text("Регистрация завершена! Изображение временно недоступно.")
    
    # Don't call start again to avoid loops
